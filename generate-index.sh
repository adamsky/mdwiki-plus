...
find $INDIR -mindepth 1 -maxdepth 1 -type d -printf "| [&#x1F4C1; %f]("$RELPATH"/%f/index.md) | %TY-%Tm-%Td %TH:%TM:%.2TS | %s |\n" >> $OUTTMP
find $INDIR -maxdepth 1 -type f \( -iname \*.markdown -o -iname \*.md -a ! -name index.md \) -printf "| [&#x1F4D5; %f]("$RELPATH"/%f) | %TY-%Tm-%Td %TH:%TM:%.2TS | %s |\n" >> $OUTTMP
